...
| **Agents & AI Apps** (Assistant용) | **Agents & AI Apps** 메뉴에서 기능 사용 설정. OAuth에 `assistant:write`, `chat:write`, `im:history` 추가. Event Subscriptions에 `assistant_thread_started`, `assistant_thread_context_changed`, `message.im` 추가. (유료 플랜 또는 [Developer Program](https://slack.com/developer-program) 샌드박스 필요할 수 있음.) |
...
9. **Assistant (어시스턴트, LLM 없음)**  
   앱 설정에서 **Agents & AI Apps** 사용 후, 앱과 **DM**을 열거나 채널에서 **앱 이름 클릭 → 어시스턴트**를 연다.  
   → 인사 메시지 + 추천 프롬프트 버튼이 보이는지, 버튼 클릭 또는 직접 입력 시 "메시지 받았어요" 고정 답장이 오는지 확인.  
   (OAuth: `assistant:write`, `chat:write`, `im:history`. Events: `assistant_thread_started`, `assistant_thread_context_changed`, `message.im`.)
